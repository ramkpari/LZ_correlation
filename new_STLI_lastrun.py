...
EEG_trigger = parallel.ParallelPort(address='0xD010')
...
for thisBlock_one in block_one:
    ...
    # keep track of which components have finished
    rsvpComponents = [Cross, Presentation_image_1, Presentation_image_2, Presentation_image_3, Presentation_image_4, EEG_trigger]
    ...
    while continueRoutine and routineTimer.getTime() > 0:
        ...
        # *EEG_trigger* updates
        if EEG_trigger.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
            # keep track of start time/frame for later
            EEG_trigger.frameNStart = frameN  # exact frame index
            EEG_trigger.tStart = t  # local t and not account for scr refresh
            EEG_trigger.tStartRefresh = tThisFlipGlobal  # on global time
            win.timeOnFlip(EEG_trigger, 'tStartRefresh')  # time at next scr refresh
            EEG_trigger.status = STARTED
            win.callOnFlip(EEG_trigger.setData, int(1))
        if EEG_trigger.status == STARTED:
            # is it time to stop? (based on global clock, using actual start)
            if tThisFlipGlobal > EEG_trigger.tStartRefresh + 1.0-frameTolerance:
                # keep track of stop time/frame for later
                EEG_trigger.tStop = t  # not accounting for scr refresh
                EEG_trigger.frameNStop = frameN  # exact frame index
                win.timeOnFlip(EEG_trigger, 'tStopRefresh')  # time at next scr refresh
                EEG_trigger.status = FINISHED
                win.callOnFlip(EEG_trigger.setData, int(0))
    ...
    if EEG_trigger.status == STARTED:
        win.callOnFlip(EEG_trigger.setData, int(0))
    block_one.addData('EEG_trigger.started', EEG_trigger.tStartRefresh)
    block_one.addData('EEG_trigger.stopped', EEG_trigger.tStopRefresh)
...
for thisBlock_two in block_two:
    ...
    if EEG_trigger.status == STARTED:
        win.callOnFlip(EEG_trigger.setData, int(0))
    block_two.addData('EEG_trigger.started', EEG_trigger.tStartRefresh)
    block_two.addData('EEG_trigger.stopped', EEG_trigger.tStopRefresh)
...
for thisBlock_three in block_three:
    ...
    if EEG_trigger.status == STARTED:
        win.callOnFlip(EEG_trigger.setData, int(0))
    block_three.addData('EEG_trigger.started', EEG_trigger.tStartRefresh)
    block_three.addData('EEG_trigger.stopped', EEG_trigger.tStopRefresh)
...
for thisBlock_four in block_four:
    ...
    if EEG_trigger.status == STARTED:
        win.callOnFlip(EEG_trigger.setData, int(0))
    block_four.addData('EEG_trigger.started', EEG_trigger.tStartRefresh)
    block_four.addData('EEG_trigger.stopped', EEG_trigger.tStopRefresh)
